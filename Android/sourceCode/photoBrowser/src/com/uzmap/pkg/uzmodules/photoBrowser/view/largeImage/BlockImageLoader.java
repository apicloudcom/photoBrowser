...
public class BlockImageLoader {
    ...
    private static Pools.SynchronizedPool<Bitmap> bitmapPool = new Pools.SynchronizedPool<Bitmap>(6);
    private Pools.SimplePool<BlockData> blockDataPool = new Pools.SimplePool<BlockData>(64);
    private Pools.SimplePool<DrawData> drawDataPool = new Pools.SimplePool<DrawData>(64);
...
//        图片没被位移，那么屏幕上显示横向2列，纵向getDisplayMetrics().heightPixels/BASEBLOCKSIZE行
        ...
        List<Position> needShowPositions = new LinkedList<Position>();
        /**
        ...
        if (loadData.currentScaleDataMap == null) {
            loadData.currentScaleDataMap = new HashMap<Position, BlockData>();
        }
        /*
        * 找出该scale所有存在的图片切块，和记录所有不存在的position
		*/
        Position positionKey = new Position();
        List<DrawData> currentDrawCaches = new ArrayList<DrawData>();
...
//        if(blockData1.size()>0)
    ...
    private List<DrawData> loadSmallDatas(LoadData loadData, int scale, List<Position> needShowPositions, int startRow, int endRow, int startCol, int endCol) {
        List<DrawData> cacheDraws = new ArrayList<DrawData>();
        if (DEBUG) {
